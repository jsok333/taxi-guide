...
2. 고객 위치에서 가장 가까운 가용 택시를 List Up하고 (미운행 차량이 없는 경우 운행차량 대상) 해당 고객위치와 고객 목적지를 전달한다.(할당 요청)
3. 해당 요청에 가장 신속히 도착 가능한 조건의 택시를 자동할당 한다.
4. 할당 즉시, 고객에게 호출완료 정보를 전달 한다.
5. 고객은 택시호출을 취소 할 수 있다.
6. 호출이 취소 되면 해당 할당을 취소하고 확인을 한다.
7. 고객은 할당확인 및 할당취소 정보를 카톡으로 받고 호출상태를 중간중간 조회 한다.(고객)

비기능적 요구사항
1. 트랜잭션
    1) 택시 할당확인 되지 않으면 고객 호출요청을 할 수 없다.  Sync 호출 
2. 장애격리
    1) 택시 할당요청은 할당확인 기능이 동작하지 않더라도, 365일 24시간 받을 수 있어야 한다 Async (event-driven), Eventual  Consistency
    2) 고객 호출요청이 과중되면 택시 할당확인 요청을 잠시동안 받지 않고 잠시후에 하도록 유도한다 Circuit breaker, fallback
3. 성능
...
![1 기능 화면 캡처 2021-02-27 232853](https://user-images.githubusercontent.com/61448505/109390281-04392600-7954-11eb-8e93-37ab5b47b9e3.png)


    - 고객이 택시를 호출한다 (ok)
    - 서비스 가능 택시들에 대한 할당을 요청한다 (ok)
    - 할당요청이 정상 처리되면 서비스 대상 택시가 할당 확인한다. (ok)
    - 할당확인 정보가 고객에게 전달된다 (ok)

    - 고객이 호출을 취소할 수 있다 (ok)
    - 호출이 취소되면 할당요청 및 할당확인이 취소된다 (ok)
    - 고객이 호출상태를 중간중간 조회한다 (View-green sticker 의 추가로 ok) 
    - 할당확인/할당취소가 발생할 때 마다 카톡으로 알림을 보낸다 (?)


### 비기능 요구사항에 대한 검증


![2 비기능 화면 캡처 2021-02-27 233023](https://user-images.githubusercontent.com/61448505/109390560-c210e400-7955-11eb-81f0-66fd6e669984.png)

    - 마이크로 서비스를 넘나드는 시나리오에 대한 트랜잭션 처리
      1) 택시 할당요청이 완료되지 않은 호출요청 완료처리는 최종 할당이 되지 않는 경우 무한정 대기 등 대고객 서비스 및 신뢰도에 치명적 문제점이 있어 ACID 트랜잭션 적용. 호출요청 시 택시 할당요청에 대해서는 Request-Response 방식 처리
      2) 호출요청 완료시 할당확인 및 결과 전송:  Taxi manage service 에서 taxi Assign 마이크로서비스로 택시할당 요청이 전달되는 과정에 있어서 taxi Assig 마이크로 서비스가 별도의 배포주기를 가지기 때문에 Eventual Consistency 방식으로 트랜잭션 처리함.
      3) 나머지 모든 inter-microservice 트랜잭션: 호출상태, 할당/할당취소 여부 등 이벤트에 대해 카톡을 처리하는 등, 데이터 일관성의 시점이 크리티컬하지 않은 모든 경우가 대부분이라 판단, Eventual Consistency 를 기본으로 채택함.
